 ...
 *
 * <p>Không cần kết nối database vì ItemFactory chỉ tạo object trong bộ nhớ. Đây là "pure unit test"
 * — nhanh, độc lập, không có side effect.
 *
 * <p>Mục tiêu kiểm tra:
 *
 * <ul>
 *   <li>Factory tạo đúng subclass tương ứng với category
 *   <li>Các thuộc tính riêng của từng subclass được gán đúng
 *   <li>Category không hợp lệ ném đúng exception
 *   <li>Các trường dùng chung (name, description, sellerId) được gán đúng
 * </ul>
 */
...
class ItemFactoryTest {
  ...
  class CreateElectronics {
    ...
    void testCreateElectronicsReturnsCorrectInstance() {
      CreateItemRequest req =
          buildRequest(
              "Laptop Dell XPS 15",
              "Laptop cao cấp dành cho lập trình viên",
              "ELECTRONICS",
              "Dell");
      ...
      assertInstanceOf(
          Electronics.class, item, "Factory phải trả về Electronics khi category = ELECTRONICS");
    ...
    void testCreateElectronicsCaseInsensitive() {
      CreateItemRequest req = buildRequest("Samsung TV", "Smart TV 4K", "electronics", "Samsung");
  ...
  class CreateArt {
    ...
    void testCreateArtReturnsCorrectInstance() {
      CreateItemRequest req =
          buildRequest(
              "Bức tranh Hồ Gươm", "Tranh sơn dầu vẽ tay, 60x80cm", "ART", "Nguyễn Văn Nghĩa");

      Item item = ItemFactory.create(req, SELLER_ID);

      assertInstanceOf(Art.class, item, "Factory phải trả về Art khi category = ART");
    ...
    void testCreateArtUnicodeArtistName() {
      ...
      CreateItemRequest req = buildRequest("Phong cảnh mùa thu", "...", "ART", unicodeName);
  ...
  class CreateVehicle {
    ...
    void testCreateVehicleReturnsCorrectInstance() {
      ...
      assertInstanceOf(Vehicle.class, item, "Factory phải trả về Vehicle khi category = VEHICLE");
    ...
    void testCreateVehicleInvalidYearFormatThrowsException() {
      CreateItemRequest req = buildRequest("Xe máy Honda", "...", "VEHICLE", "hai-nghìn-hai-mươi");
  ...
  class InvalidCategory {
    ...
    void testInvalidCategoryThrowsIllegalArgumentException() {
      CreateItemRequest req = buildRequest("Sản phẩm lạ", "...", "FURNITURE", "gỗ sồi");
  ...
  class PolymorphismTest {
    ...
    void testGetCategoryPolymorphism() {
      Item electronics =
          ItemFactory.create(buildRequest("TV", "...", "ELECTRONICS", "Sony"), SELLER_ID);
      Item art = ItemFactory.create(buildRequest("Tranh", "...", "ART", "Picasso"), SELLER_ID);
      Item vehicle = ItemFactory.create(buildRequest("Xe", "...", "VEHICLE", "2020"), SELLER_ID);

      assertAll(
          "getCategory() polymorphism",
