...
/**
 * Test suite verifying the contract of {@link UnauthorizedException} — thrown when a caller
 * lacks the required permissions to perform an action, such as bidding under another user's
 * identity, modifying an item they do not own, or presenting an expired/invalid JWT token.
 * Typically maps to an HTTP 401 or 403 response at the API layer.
 *
 ...
 *       {@code UnauthorizedException → AuctionException → RuntimeException}.</li>
...
class UnauthorizedExceptionTest {
   ...
   *
   * <p>Cause chaining is particularly relevant here because {@link UnauthorizedException}
   * is often raised in response to a JWT verification failure (e.g., {@link SecurityException}
   * from the token library). Wrapping the original exception preserves the authentication
  ...
  /**
   * Verifies that {@link UnauthorizedException} is a subtype of {@link AuctionException},
  ...
  /**
   * Verifies that {@link UnauthorizedException} extends {@link RuntimeException}, meaning
