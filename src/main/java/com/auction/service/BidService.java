...
public class BidService {
    ...
    public BidTransaction placeBid(Long auctionId, BidRequest request, Long bidderId) {
        ...
        BidTransaction savedBid = jdbi.inTransaction(handle -> {
            ...
            LOGGER.info("Bid placed: auction={}, bidder={}, amount={}",
        ...
        if (broadcaster != null) {
            try {
                String username = userDao.findById(bidderId)
                    .map(User::getUsername)
                    .orElse("Unknown");

                Auction updatedAuction = auctionDao.findById(auctionId).orElse(null);
                if (updatedAuction != null) {
                    BidUpdateMessage message = BidUpdateMessage.bidUpdate(
                        auctionId,
                        request.getAmount(),
                        bidderId,
                        username,
