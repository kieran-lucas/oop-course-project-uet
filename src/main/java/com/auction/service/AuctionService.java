...
public class AuctionService {
     ...
     *
     * <p>GET /api/auctions?status=RUNNING → chỉ lấy auctions đang chạy.
     * GET /api/auctions → lấy tất cả.
     */
     ...
     *
     * <p>AuctionResponse chứa nhiều thông tin hơn Auction model:
     * - itemName, itemCategory: lấy từ bảng items
     * - leadingBidderUsername: lấy từ bảng users
     *
     ...
     * 2. Item có thuộc seller này không?
     * 3. startTime < endTime?
     * 4. startingPrice > 0?
     *
    ...
    public Auction createAuction(CreateAuctionRequest request, Long userId, String role) {
        ...
        if (request.getStartingPrice() == null
            || request.getStartingPrice().signum() <= 0) {
        ...
        LOGGER.info("Auction created: id={}, itemId={}, seller={}",
            saved.getId(), saved.getItemId(), userId);
        return saved;
    }
     ...
     *
     * <p>Chỉ cho phép sửa khi status = OPEN (chưa bắt đầu).
     * Khi đã RUNNING hoặc FINISHED thì không sửa được.
     *
     ...
     *
     * <p>AuctionResponse.fromAuction() chỉ copy dữ liệu từ Auction model.
     * Method này bổ sung thêm thông tin từ bảng items và users.
     */
    private AuctionResponse enrichAuctionResponse(Auction auction) {
        ...
        if (auction.getLeadingBidderId() != null) {
            userDao.findById(auction.getLeadingBidderId()).ifPresent(user -> {
