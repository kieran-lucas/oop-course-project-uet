 ...
 *
 * <p><b>Vấn đề cần giải quyết:</b> Server nhận JSON từ client chứa trường {@code category} là
 * String ("ELECTRONICS", "ART", "VEHICLE"). Làm thế nào để tạo đúng subclass Java ({@code
 * Electronics}, {@code Art}, {@code Vehicle}) mà không làm lộ logic tạo object ra khắp nơi trong
 * codebase?
 *
 * <p><b>Giải pháp Factory Method:</b> Tập trung toàn bộ logic quyết định subclass vào một điểm duy
 * nhất — phương thức {@link #create}. Caller ({@code ItemService}) chỉ gọi {@code
 * ItemFactory.create(req, sellerId)} mà không cần biết nội bộ hoạt động thế nào.
 *
 ...
 * </pre>
 *
 * <p><b>Lợi ích khi thêm category mới:</b> Chỉ cần thêm 1 case vào switch trong {@link #create} và
 * tạo subclass mới. Không cần sửa {@code ItemService}, {@code ItemController}, hay bất kỳ caller
 * nào khác — đây là nguyên tắc Open/Closed Principle.
 *
 * <p><b>Luồng dữ liệu:</b>
 *
 * <pre>
...
public class ItemFactory {
  ...
  private ItemFactory() {
    throw new UnsupportedOperationException(
        "ItemFactory là utility class, không khởi tạo instance");
  }
   ...
   *
   * <ul>
   *   <li>{@code ELECTRONICS}: categoryDetail = tên thương hiệu (brand), ví dụ "Samsung"
   *   <li>{@code ART}: categoryDetail = tên nghệ sĩ (artist), ví dụ "Van Gogh"
   *   <li>{@code VEHICLE}: categoryDetail = năm sản xuất dạng String, ví dụ "2020"
   * </ul>
   *
   ...
   * @throws NumberFormatException nếu category là VEHICLE nhưng categoryDetail không parse được
   *     sang int
   */
  public static Item create(CreateItemRequest req, Long sellerId) {
    ...
    return switch (category) {
      case "ELECTRONICS" ->
          new Electronics(
              req.getName(), req.getDescription(), sellerId, req.getCategoryDetail() // brand
              );

      case "ART" ->
          new Art(
              req.getName(), req.getDescription(), sellerId, req.getCategoryDetail() // artist
              );
  ...
  private static int parseYear(String yearStr, String itemName) {
    ...
    if (year < 1886 || year > 2100) {
      throw new IllegalArgumentException(
          "Năm sản xuất không hợp lệ cho '" + itemName + "': " + year + " (hợp lệ: 1886–2100)");
    }
    return year;
  }
}
