 ...
 *
 * <p>Đây là trạng thái khởi đầu của vòng đời phiên đấu giá. Trong giai đoạn này, seller
 * vẫn còn quyền điều chỉnh thông tin (giá khởi điểm, thời gian, mô tả...) hoặc hủy bỏ
 * hoàn toàn nếu đổi ý. Mọi hành vi liên quan đến quá trình đấu giá thật sự (đặt giá,
 * gia hạn) đều chưa được phép vì chưa có người tham gia.
 *
 ...
 * <ul>
 *   <li>OPEN → RUNNING: tự động khi đồng hồ chạm {@code startTime}, do
 *       {@code AuctionScheduler} kích hoạt.</li>
 *   <li>OPEN → CANCELED: khi seller hoặc admin chủ động hủy phiên.</li>
 * </ul>
 *
...
public class OpenState implements AuctionState {
     ...
     * {@inheritDoc}
     *
     * <p>Từ chối mọi yêu cầu đặt giá cho đến khi phiên chính thức bắt đầu. Người dùng
     * cần chờ tới {@code startTime} của phiên.
     *
     ...
     * {@inheritDoc}
     *
     * <p>Cho phép đóng (hủy) phiên khi còn ở trạng thái OPEN. Method này chỉ kiểm tra
     * tính hợp lệ về mặt trạng thái; việc cập nhật trường {@code status} thực sự do
     * {@code AuctionService} thực hiện sau khi method trả về.
     *
     ...
     * {@inheritDoc}
     *
     * <p>Cho phép chỉnh sửa các trường thông tin của phiên vì chưa có người tham gia.
     * Seller có thể thay đổi giá khởi điểm, thời gian bắt đầu/kết thúc, mô tả...
     *
     ...
     * {@inheritDoc}
     *
     * <p>Từ chối gia hạn vì phiên chưa diễn ra. Cơ chế anti-sniping chỉ áp dụng khi
     * phiên đang ở {@link RunningState}.
     *
