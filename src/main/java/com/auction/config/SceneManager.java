 ...
 *
 * Kiến trúc: Single Scene + StackPane swap (giống SPA trong web).
 * - 1 Scene duy nhất, không bao giờ tạo/hủy Scene
 * - 1 StackPane root, chuyển màn = swap children
 * - FXML lazy load lần đầu, cache vĩnh viễn trong HashMap
 * - Controller cũng cache → giữ state khi quay lại
 * - CSS load 1 lần gắn vào Scene → tất cả FXML đều có theme
 *
 * Sử dụng:
...
public class SceneManager {
     ...
     *
     * Luồng:
     * 1. Gọi onNavigatedFrom() trên controller hiện tại (nếu có)
     * 2. Kiểm tra cache → nếu chưa có thì lazy load FXML
     * 3. Swap view trong StackPane (instant, < 5ms)
     * 4. Gọi onNavigatedTo() trên controller mới
     *
     ...
     *
     * Ví dụ: từ auction-list, click vào 1 auction:
     *   SceneManager.getInstance().navigateTo("auction-detail.fxml", auctionId);
     *
     * Controller đích nhận data trong onDataReceived():
     *   public void onDataReceived(Object data) {
     *       Long auctionId = (Long) data;
    ...
    /**
     * Gọi onNavigatedFrom() trên controller đang hiển thị.
     * Controller dùng method này để cleanup (đóng WebSocket, hủy Timer...).
     */
     ...
     * Dùng khi cần force reload FXML (ví dụ: sau khi thay đổi role).
     */
    ...
    /**
     * Lấy controller đã cache theo tên FXML.
     * Dùng khi cần giao tiếp giữa 2 controller.
     *
     * Ví dụ: từ CreateAuctionController cần refresh AuctionListController:
     *   AuctionListController ctrl = (AuctionListController)
     *       SceneManager.getInstance().getController("auction-list.fxml");
     *   ctrl.refreshList();
     */
