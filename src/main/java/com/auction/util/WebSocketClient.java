 ...
 *
 * <p><b>Mục đích:</b>
 * Kết nối WebSocket đến {@code ws://localhost:8080/ws/auction/{id}?token=<jwt>},
 * nhận JSON message từ server (BID_UPDATE, TIME_EXTENDED, AUCTION_ENDED),
 * và gọi callback để AuctionDetailController xử lý cập nhật UI.
 *
 * <p><b>Các phương thức chính:</b>
 * <ul>
 *   <li>{@link #connect(Long, String, Consumer)} — Mở kết nối WS, đăng ký callback nhận message.</li>
 *   <li>{@link #disconnect()} — Đóng kết nối WS (gọi khi rời màn hình auction-detail).</li>
 *   <li>{@link #isConnected()} — Kiểm tra trạng thái kết nối.</li>
 * </ul>
 *
 * <p><b>Vị trí trong kiến trúc:</b>
 * WebSocketClient được dùng bởi {@code AuctionDetailController} để nhận
 * realtime bid updates từ server. Kết nối được mở khi vào màn hình chi tiết
 * và đóng khi rời màn hình (qua {@code Navigable.onNavigatedFrom()}).
 *
 * <p><b>Luồng dữ liệu:</b>
 * <pre>
...
public class WebSocketClient {
  ...
  /**
   * Đóng kết nối WebSocket nếu đang mở.
   * Nên gọi trong {@code Navigable.onNavigatedFrom()} để giải phóng tài nguyên.
   */
