 ...
 * <ul>
 *   <li>JWT Middleware (cấu hình trong {@link com.auction.App}) đã xác thực token
 *       và set các attribute vào context: {@code userId}, {@code username}, {@code role}.</li>
 *   <li>Controller đọc {@code ctx.attribute("role")} để kiểm tra role.</li>
 *   <li>Controller đọc {@code ctx.attribute("userId")} để kiểm tra ownership.</li>
 * </ul>
 *
 * <p>Factory Method pattern được áp dụng trong {@link ItemService#create}: nhận
 * {@code category} từ request ("ELECTRONICS"/"ART"/"VEHICLE"), tạo đúng subclass tương ứng.
 *
...
public class ItemController {
    ...
    public static void register(Javalin app, ItemService itemService) {
        ...
        LOGGER.info("Đã đăng ký ItemController: GET/POST /api/items, "
     ...
     * <ul>
     *   <li>{@code ?sellerId=5} — chỉ trả về item của seller có id=5.</li>
     *   <li>Không có parameter → trả về tất cả item trong hệ thống.</li>
     * </ul>
     *
     * <p>Ví dụ response:
     * <pre>
     ...
     *
     * <p>Nếu không tìm thấy item với ID đã cho, service sẽ ném {@code NotFoundException}
     * và exception handler trong {@code App.java} sẽ trả về HTTP 404.
     *
     * @param ctx         Javalin context chứa HTTP request/response
     ...
     *
     * <p>Factory Method pattern được áp dụng ở đây: {@code ItemService.create()} nhận
     * {@code category} từ request và gọi {@code ItemFactory.create()} để tạo đúng subclass
     * ({@code Electronics}, {@code Art}, hoặc {@code Vehicle}).
     *
     * <p>Luồng xử lý:
     * <ol>
     *   <li>Kiểm tra {@code role == "SELLER"} từ JWT context — ném
     *       {@code UnauthorizedException} nếu không phải SELLER.</li>
     *   <li>Lấy {@code userId} từ JWT context để gán làm {@code sellerId}.</li>
     *   <li>Parse {@link CreateItemRequest} từ body.</li>
     *   <li>Gọi {@code ItemService.create()} → Factory tạo đúng subclass → lưu DB.</li>
     * </ol>
     *
    ...
    private static void handleCreate(Context ctx, ItemService itemService) {
        ...
        LOGGER.info("Tạo item mới: sellerId={}, category={}, name={}",
            sellerId, request.getCategory(), request.getName());
     ...
     *
     * <p>Kiểm tra ownership: {@code item.sellerId == userId từ JWT}. Nếu một seller
     * cố sửa item của seller khác → {@code UnauthorizedException}.
     *
     * <p>Lưu ý: Item đang được dùng trong phiên đấu giá RUNNING sẽ KHÔNG thể sửa
     * (logic này được xử lý ở tầng Service, không phải Controller).
     *
     * @param ctx         Javalin context chứa HTTP request/response
     ...
     * <p>Logic phân quyền:
     * <ul>
     *   <li>Role {@code ADMIN} → có thể xóa bất kỳ item nào.</li>
     *   <li>Role {@code SELLER} → chỉ xóa được item của mình.</li>
     *   <li>Role {@code BIDDER} → không được phép → {@code UnauthorizedException}.</li>
     * </ul>
     *
     * <p>Nếu item đang liên kết với phiên đấu giá RUNNING, service sẽ ném lỗi
     * để tránh mất tính toàn vẹn dữ liệu.
     *
     * @param ctx         Javalin context chứa HTTP request/response
