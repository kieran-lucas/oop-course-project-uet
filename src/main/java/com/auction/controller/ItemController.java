 ...
 *
 * <ul>
 *   <li>GET /api/items — list tất cả items (public, filter theo sellerId)
 *   <li>GET /api/items/:id — chi tiết 1 item
 *   <li>POST /api/items — seller tạo item (cần JWT, role=SELLER)
 *   <li>PUT /api/items/:id — seller sửa item (cần JWT, kiểm tra ownership)
 *   <li>DELETE /api/items/:id — seller/admin xóa item
 * </ul>
 *
 * <p>Controller KHÔNG chứa logic nghiệp vụ — chỉ: 1. Đọc request (path param, query param, body) 2.
 * Đọc thông tin user từ JWT (ctx.attribute) 3. Gọi Service 4. Trả response
 */
