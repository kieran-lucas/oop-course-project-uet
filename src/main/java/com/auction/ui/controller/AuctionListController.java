 ...
 *
 * <p><b>Mục đích:</b> Hiển thị danh sách tất cả phiên đấu giá, hỗ trợ tìm kiếm/lọc theo trạng thái,
 * tự động làm mới dữ liệu mỗi 10 giây. Seller thấy thêm nút "+ Tạo phiên".
 *
 * <p><b>Các phương thức chính:</b>
 *
 * <ul>
 *   <li>{@link #onNavigatedTo()} — Load danh sách phiên từ API khi vào màn hình.
 *   <li>{@link #handleSearch()} — Lọc danh sách theo từ khóa và trạng thái.
 *   <li>{@link #handleRefresh()} — Tải lại dữ liệu từ server.
 *   <li>{@link #handleCreateAuction()} — Chuyển sang màn hình tạo phiên (chỉ SELLER).
 *   <li>{@link #handleLogout()} — Đăng xuất và quay về welcome.fxml.
 * </ul>
 *
 * <p><b>Vị trí trong kiến trúc:</b> AuctionListController là màn hình chính sau đăng nhập. Nó gọi
 * {@link RestClient} để lấy dữ liệu từ {@code GET /api/auctions}, hiển thị vào TableView, và điều
 * hướng đến auction-detail.fxml khi người dùng click "Vào".
 */
public class AuctionListController implements Navigable {
  ...
  public void onNavigatedTo() {
    ...
    // Hiển thị chuông thông báo cho BIDDER và SELLER
    ...
    if (showBell) {
      updateBadge();
      // Remove any stale listener before re-registering (guards against double-registration)
  ...
  /** Lọc danh sách theo từ khóa, trạng thái và danh mục đã chọn. */
  @FXML
            ...
            .filter(
                a -> {
                  ...
                  boolean matchCategory =
                      category == null || "Tất cả".equals(category)
  ...
  public void handleBellClick() {
    ...
    popup.show(stage, bounds.getMinX() - content.getPrefWidth() + bounds.getWidth(),
        bounds.getMaxY() + 6);
  }
  ...
  private void updateCategoryFilter(List<AuctionResponse> auctions) {
    ...
    // Always include all known item categories
    for (String cat : new String[]{"ART", "ELECTRONICS", "VEHICLE"}) {
      categories.add(cat);
    }
    // Add any extra categories that arrive from the server but aren't in the known set
    auctions.stream()
  ...
  private void setupStatusFilter() {
    statusFilter.setItems(
        FXCollections.observableArrayList("Tất cả", "OPEN", "RUNNING", "FINISHED", "CANCELED", "PAID"));
  }
