 ...
 *   <li><b>Xác định người thắng:</b> Tìm bid cuối cùng của phiên đã kết thúc</li>
 ...
 * </ul>
 * 
 * <h3>Bất biến (append-only)</h3>
 * <p>BidTransaction KHÔNG BAO GIỜ bị sửa hay xóa. Nếu có lỗi (ví dụ: bid do bug),
 * hệ thống sẽ tạo một bid điều chỉnh (adjustment) thay vì sửa bid cũ.
 * 
...
public class BidTransactionDao {
    ...
    /**
     * Ghi lại một bid thành công vào database.
     * 
     ...
     * </pre>
     * 
     * <p>Nếu có lỗi sau khi insert, toàn bộ transaction rollback → không có
     * bid transaction lẻ loi mà không có auction update tương ứng.
     * 
    ...
    /**
     * Ghi lại một bid thành công (phiên bản không cần Handle, tạo transaction riêng).
     * 
     * <p>Dùng cho các trường hợp không cần transaction với auction update,
     * ví dụ: khi đang insert bid history cho mục đích logging không critical.
     * 
     ...
     * 
     * <p>Client JavaFX sẽ gọi GET /api/auctions/{id}/bids,
     * Controller gọi method này và trả về List&lt;BidTransaction&gt; cho client.
     * 
     ...
     * @return List các BidTransaction của user đó
     */
     ...
     * Bid cuối cùng chính là bid có giá cao nhất và thời gian muộn nhất.
     * 
     * @param auctionId ID của phiên đấu giá
     * @return Optional chứa BidTransaction cuối cùng, Optional.empty() nếu chưa có bid nào
     */
    public java.util.Optional<BidTransaction> findLastBid(Long auctionId) {
        String sql = """
            SELECT id, auction_id, bidder_id, amount, auto_bid, created_at
            FROM bid_transactions
            WHERE auction_id = :auctionId
     ...
     * 
     * <p><b>Lưu ý:</b> Method này trả về một Object chứa cả BidTransaction và username.
     * Có thể tạo một DTO riêng (BidHistoryEntry) hoặc dùng Map.
     * 
     * @param auctionId ID của phiên đấu giá
     * @return List các Object[] chứa [BidTransaction, username]
     */
     ...
     */
    public java.util.Optional<BigDecimal> getHighestPrice(Long auctionId) {
        String sql = """
            SELECT amount
            FROM bid_transactions
            WHERE auction_id = :auctionId
     ...
     * 
     * <p>Được dùng trong findWithUsernames() để trả về cả bid transaction
     * và tên người dùng trong một object duy nhất.
     */
    public static class BidHistoryEntry {
        private final BidTransaction transaction;
        private final String username;
        
        public BidHistoryEntry(BidTransaction transaction, String username) {
