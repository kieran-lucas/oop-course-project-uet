 ...
 *
 * <ul>
 *   <li>category = "ELECTRONICS" → new Electronics(..., brand, ...)
 *   <li>category = "ART" → new Art(..., artist, ...)
 *   <li>category = "VEHICLE" → new Vehicle(..., year, ...)
 * </ul>
 *
 ...
 *
 * <p>ItemFactory (trong pattern/factory/) tạo Item subclass từ CreateItemRequest. DAO chỉ đọc/ghi,
 * không quyết định logic tạo object.
 *
 * <h3>Liên kết với các file khác</h3>
 *
 * <ul>
 *   <li><b>Item.java, Electronics.java, Art.java, Vehicle.java</b> — model classes
 *   <li><b>ItemService.java</b> — gọi các method của DAO này
 *   <li><b>ItemFactory.java</b> — tạo subclass từ request
 *   <li><b>AuctionDao.java</b> — auctions tham chiếu đến items (item_id foreign key)
 *   <li><b>V1__initial_schema.sql</b> — định nghĩa bảng items
 * </ul>
 */
public class ItemDao {
   ...
   *
   * <p>Đây là nơi thể hiện POLYMORPHISM khi đọc từ database. Dựa vào cột "category", mapper tạo
   * đúng subclass:
   *
   * <ul>
   *   <li>ELECTRONICS: đọc thêm cột "brand"
   *   <li>ART: đọc thêm cột "artist"
   *   <li>VEHICLE: đọc thêm cột "year"
   * </ul>
   *
  ...
  private static class ItemMapper implements RowMapper<Item> {
    ...
    public Item map(ResultSet rs, StatementContext ctx) throws SQLException {
      ...
      return switch (category) {
        case "ELECTRONICS" ->
            new Electronics(id, name, description, sellerId, rs.getString("brand"), createdAt);
        case "ART" -> new Art(id, name, description, sellerId, rs.getString("artist"), createdAt);
        case "VEHICLE" ->
            new Vehicle(id, name, description, sellerId, rs.getInt("year"), createdAt);
        default ->
            throw new IllegalStateException(
                "Category không hợp lệ trong database: '"
                    + category
                    + "' (item id="
                    + id
                    + ", name="
   ...
   *
   * <p>Method này xử lý cả 3 loại sản phẩm (Electronics, Art, Vehicle). Tùy vào subclass thực tế
   * của item, nó sẽ lấy các field đặc thù tương ứng.
   *
   * <p>Luồng xử lý:
   *
   * <ol>
   *   <li>ItemService nhận CreateItemRequest, dùng ItemFactory tạo Item subclass
   *   <li>Gọi itemDao.insert(item) với item có id = null
   *   <li>JDBI thực thi INSERT với các field phù hợp
   *   <li>Trả về item đã có id
   * </ol>
   *
  ...
  public Item insert(Item item) {
    ...
    return jdbi.withHandle(
        handle -> {
          ...
          item.setId(id);
          LOGGER.debug(
              "Inserted item: id={}, name={}, category={}, seller={}",
              id,
              item.getName(),
              item.getCategory(),
              item.getSellerId());
   ...
   *
   * <p>[FIX #2] Đổi kiểu trả về từ {@code int} sang {@code Integer} (boxed). Trả về {@code null}
   * cho non-Vehicle items → cột year trong DB nhận NULL thay vì giá trị 0 vô nghĩa (Electronics
   * không có năm sản xuất).
   *
   ...
   *
   * <ul>
   *   <li>Seller xem danh sách sản phẩm của mình
   *   <li>Khi tạo auction, chỉ hiển thị items của seller đó
   * </ul>
   *
   ...
   *
   * <p>[FIX #8] Escape ký tự đặc biệt của LIKE (%, _) trong keyword. Tránh lỗi khi user nhập ký tự
   * đặc biệt vào ô tìm kiếm. Ví dụ: search "100%" sẽ chỉ match items có đúng chuỗi "100%" thay vì
   * match mọi item bắt đầu bằng "100".
   *
   ...
   *
   * <p>Seller có thể sửa tên, mô tả, và các field đặc thù của sản phẩm. Không cho phép sửa
   * seller_id (không thể chuyển sản phẩm sang người khác).
   *
   ...
   * Xóa sản phẩm.
   *
   * <p><b>Lưu ý:</b> Do có ON DELETE CASCADE trong bảng auctions, khi xóa item, tất cả phiên đấu
   * giá liên quan cũng bị xóa.
   *
  ...
  public int deleteBySellerId(Long sellerId) {
    ...
    int rowsAffected =
        jdbi.withHandle(handle -> handle.createUpdate(sql).bind("sellerId", sellerId).execute());
  ...
  public int countBySellerId(Long sellerId) {
    ...
    return jdbi.withHandle(
        handle -> handle.createQuery(sql).bind("sellerId", sellerId).mapTo(Integer.class).one());
  }
}
