...
public class DepositRequestDao {
  ...
  public Optional<DepositRecord> findById(Long id) {
    String sql = """
        ...
        """;
    return jdbi.withHandle(handle ->
        handle.createQuery(sql).bind("id", id)
            .map(new DepositRecordMapper()).findOne());
  }
  ...
  public List<DepositRecord> findByStatus(String status) {
    String sql = """
        ...
        """;
    return jdbi.withHandle(handle ->
        handle.createQuery(sql).bind("status", status)
            .map(new DepositRecordMapper()).list());
  }
  ...
  public boolean updateStatus(Long id, String newStatus) {
    ...
    int rows = jdbi.withHandle(handle ->
        handle.createUpdate(sql).bind("status", newStatus).bind("id", id).execute());
    return rows > 0;
  }
}
