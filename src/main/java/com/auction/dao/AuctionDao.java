 ...
 *
 * <p>Class này chịu trách nhiệm giao tiếp với bảng auctions trong database.
 * Đây là DAO quan trọng nhất vì nó xử lý các thao tác đấu giá cốt lõi,
 * bao gồm cả việc khóa row để tránh race condition (SELECT FOR UPDATE).
 *
 * <h3>Vai trò trong hệ thống</h3>
 * <p>AuctionDao nằm ở tầng dưới cùng, được AuctionService và BidService gọi.
 * Mọi thao tác đọc/ghi trên bảng auctions đều đi qua DAO này.
 *
 * <h3>Concurrency - SELECT FOR UPDATE</h3>
 * <p>Khi nhiều user bid cùng lúc, cần đảm bảo không có lost update.
 * Giải pháp: 2 tầng bảo vệ
 * <ol>
 *   <li><b>Tầng application:</b> synchronized(auction) trong BidService</li>
 *   <li><b>Tầng database:</b> SELECT ... FOR UPDATE khóa row khi đọc</li>
 * </ol>
 *
 ...
 * <p>Trạng thái của Auction (OPEN, RUNNING, FINISHED, PAID, CANCELED)
 * được quản lý bởi State pattern trong service layer.
 * DAO chỉ đơn thuần đọc/ghi trạng thái, không quyết định logic chuyển trạng thái.
 *
 * <h3>Liên kết với các file khác</h3>
 * <ul>
 *   <li><b>Auction.java</b> — model class, chứa dữ liệu phiên đấu giá</li>
 *   <li><b>BidService.java</b> — gọi findByIdForUpdate() và updateInTransaction()</li>
 *   <li><b>AuctionService.java</b> — CRUD thông thường cho auction</li>
 *   <li><b>AuctionScheduler.java</b> — cập nhật trạng thái khi hết giờ</li>
 *   <li><b>V1__initial_schema.sql</b> — định nghĩa bảng auctions</li>
 * </ul>
 */
public class AuctionDao {
   ...
   *
   * <p>Bao gồm cả updated_at [FIX #6] — trước đây SELECT có updated_at
   * nhưng Mapper không đọc, gây mất thông tin cập nhật cuối cùng.
   */
   ...
   *
   * <p>[FIX #6] Mapper giờ đọc cả cột updated_at và set vào Auction object.
   * Trước đây cột này được SELECT nhưng bị bỏ qua → mất dữ liệu "cập nhật lần cuối".
   *
   * <p><b>LƯU Ý:</b> Cần thêm field {@code updatedAt} (kiểu LocalDateTime) và setter
   * {@code setUpdatedAt()} vào model {@code Auction.java} nếu chưa có.
   *
   ...
   * <ul>
   *   <li>GET /api/auctions/{id} — xem chi tiết phiên</li>
   *   <li>Kiểm tra phiên tồn tại trước khi xóa</li>
   * </ul>
   *
   ...
   *
   * <p><b>QUAN TRỌNG:</b> Method này phải được gọi TRONG TRANSACTION.
   * Row bị khóa cho đến khi transaction COMMIT hoặc ROLLBACK.
   * Các transaction khác cố gắng đọc row này sẽ phải chờ.
   *
   ...
   * <p>Dùng cho:
   * <ul>
   *   <li>AuctionScheduler: tìm các phiên RUNNING sắp hết giờ</li>
   *   <li>Admin Panel: lọc phiên theo trạng thái</li>
   *   <li>Seller: xem phiên đang chạy của mình</li>
   * </ul>
   *
  ...
  public List<Auction> findByStatus(String status) {
    String sql = "SELECT " + SELECT_COLUMNS
        + " FROM auctions WHERE status = :status ORDER BY end_time ASC";
   ...
   *
   * <p>Theo logic, mỗi item chỉ nên có 1 phiên đấu giá, nhưng để an toàn
   * vẫn hỗ trợ lấy danh sách (trong trường hợp item được đem đấu giá lại).
   *
   ...
   *
   * <p>[FIX #5] Đổi tên từ {@code update(Handle, Auction)} thành
   * {@code updateInTransaction(Handle, Auction)} để phân biệt rõ hành vi:
   * <ul>
   *   <li>{@code updateInTransaction} — <b>throw exception</b> nếu không tìm thấy
   *       (fail-fast trong transaction, rollback toàn bộ)</li>
   *   <li>{@code update} — <b>return false</b> nếu không tìm thấy
   *       (caller tự xử lý)</li>
   * </ul>
   *
   ...
   *
   * <p>[FIX #5] Hành vi khác so với {@code updateInTransaction}:
   * return false nếu không tìm thấy, KHÔNG throw exception.
   *
   * <p>Dùng cho các thao tác update đơn giản không cần transaction,
   * ví dụ: Admin sửa trạng thái, AuctionScheduler đóng phiên.
   *
   * @param auction Auction đã được cập nhật (phải có id)
  ...
  public boolean update(Auction auction) {
    ...
    if (rowsAffected > 0) {
      LOGGER.debug("Updated auction: id={}, status={}, price={}",
   ...
   *
   * <p>Được gọi bởi AuctionScheduler mỗi phút để chuyển các phiên hết hạn
   * từ trạng thái RUNNING sang FINISHED.
   *
